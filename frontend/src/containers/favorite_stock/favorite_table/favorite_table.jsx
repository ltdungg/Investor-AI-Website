    ...
    return lists.map((stock, index) => {
      ...
      return (
        <tr key={symbol} onClick={() => navigate(`/stocks/${symbol}`)}>
          ...
          <td className="more-area" onClick={(e) => handleClick(e, symbol)}>
            <button className="fa-stock-btn" children={<MdMoreVert />} />
            {open === symbol && (
              <div className="more-box">
                <div
                  className="delete-stock"
                  children="Xóa cổ phiếu"
                  onClick={() => handleDeleteStock(symbol, index)}
                />
              </div>
            )}
          </td>
